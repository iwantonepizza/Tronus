...
function MatchDetailContent({
  ...
  const [mockMatch, setMockMatch] = useState(initialMatch)
  ...
  const handleCancelSession = async () => {
    ...
    if (USE_MOCKS) {
      setMockMatch((current) => ({
        ...current,
        status: 'cancelled',
      }))
      setToastState({ message: 'Партия отменена.', type: 'success' })
      return
    }
    ...
    try {
      ...
      setToastState({
        message: 'Не удалось отменить партию. Попробуйте ещё раз.',
        type: 'error',
      })
    }
  }
