...
def _attach_current_avatar(*, user: User, faction: Faction, settings, tmp_path) -> AvatarAsset:
    ...
    user.profile.save(update_fields=["current_avatar"])
    return avatar
...
@pytest.mark.django_db
def test_register_endpoint_auto_activates_with_secret_word(api_client, settings) -> None:
    ...
    assert response.json()["auto_activated"] is True
    assert user.is_active is True


@pytest.mark.django_db
def test_register_endpoint_ignores_wrong_secret_word(api_client, settings) -> None:
    settings.REGISTRATION_SECRET_WORD = "lovecraft"
    ...
    user = User.objects.get(email="pending2@example.com")
...
@pytest.mark.django_db
def test_register_endpoint_rejects_password_repeat_mismatch(api_client) -> None:
    response = api_client.post(
        "/api/v1/auth/register/",
        {
            "email": "mismatch@example.com",
            "password": "StrongPassword123!",
            "password_repeat": "WrongPassword123!",
            "nickname": "Mismatch",
        },
        format="json",
    )
    ...
    assert response.json()["password_repeat"] == ["Пароли не совпадают."]
...
@pytest.mark.django_db
def test_login_unknown_login_returns_invalid_credentials_without_authenticate(api_client) -> None:
    with patch("apps.accounts.views.authenticate") as authenticate_mock:
        response = api_client.post(
            "/api/v1/auth/login/",
            {"login": "missing@example.com", "password": "StrongPassword123!"},
            format="json",
        )
...
@pytest.mark.django_db
def test_password_reset_with_valid_secret_word_changes_password(api_client, settings) -> None:
    settings.REGISTRATION_SECRET_WORD = "lovecraft"
...
@pytest.mark.django_db
def test_password_reset_with_wrong_secret_word_returns_validation_error(
    api_client, settings
) -> None:
    settings.REGISTRATION_SECRET_WORD = "lovecraft"
    User.objects.create_user(
    ...
    assert response.json()["error"]["details"]["login"] == ["Неверный логин или секретное слово."]


@pytest.mark.django_db
def test_password_reset_for_missing_login_returns_same_validation_error(
    api_client, settings
) -> None:
    settings.REGISTRATION_SECRET_WORD = "lovecraft"
...
@pytest.mark.django_db
def test_password_change_with_valid_current_password_allows_relogin(api_client) -> None:
    User.objects.create_user(
        username="changer@example.com",
        email="changer@example.com",
        password="StrongPassword123!",
        is_active=True,
    )
    assert api_client.login(
        username="changer@example.com", password="StrongPassword123!"
    )
    ...
    relogin_response = api_client.post(
        "/api/v1/auth/login/",
        {"login": "changer@example.com", "password": "NewStrongPassword123!"},
        format="json",
    )
...
@pytest.mark.django_db
def test_password_change_with_wrong_current_password_returns_validation_error(
    api_client,
) -> None:
    User.objects.create_user(
        username="changer@example.com",
    ...
    assert response.status_code == 400
    assert response.json()["error"]["code"] == "validation_error"
    assert response.json()["error"]["details"]["current_password"] == [
...
def test_approved_user_can_login_and_read_me(api_client) -> None:
    ...
    login_response = api_client.post(
        "/api/v1/auth/login/",
        {"login": "approved@example.com", "password": "StrongPassword123!"},
        format="json",
    )
...
@pytest.mark.django_db
def test_approved_user_can_login_by_nickname(api_client) -> None:
    user = User.objects.create_user(
        username="approved@example.com",
        email="approved@example.com",
        password="StrongPassword123!",
    ...
    response = api_client.post(
        "/api/v1/auth/login/",
        {"login": "approveduser", "password": "StrongPassword123!"},
        format="json",
    )

    assert response.status_code == 200
    assert response.json()["nickname"] == "ApprovedUser"
    assert response.json()["email"] == "approved@example.com"
...
def test_login_rate_limit_blocks_sixth_request(api_client) -> None:
    ...
    for _ in range(6):
        response = api_client.post(
            "/api/v1/auth/login/",
            {"login": "ratelimit@example.com", "password": "wrong-password"},
            format="json",
...
@pytest.mark.django_db
def test_user_detail_returns_absolute_current_avatar_url(api_client, settings, tmp_path) -> None:
    user = User.objects.create_user(
    ...
    user.profile.nickname = "AvatarUser"
    user.profile.save(update_fields=["nickname"])
    faction = _ensure_stark_faction()
    _attach_current_avatar(user=user, faction=faction, settings=settings, tmp_path=tmp_path)

    response = api_client.get(f"/api/v1/users/{user.pk}/")

    assert response.status_code == 200
    assert response.json()["current_avatar"].startswith("http://testserver/media/avatars/")
